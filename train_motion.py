...
def train(epoch):
    ...
    for batch_idx, batch in tqdm(enumerate(training_data_loader, 1),
        ...
        writer.add_scalar("Loss/train_huber", loss_huger, epoch * len(training_data_loader) + batch_idx)
        writer.add_images("predicted/train_sa_mid", torch.from_numpy(sa_mid_pred_t).unsqueeze(0).unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("gt/train_sa_mid", con_sa[:, 4:5, :, :],
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("predicted/train_2ch", torch.from_numpy(la_2ch_pred_t).unsqueeze(0).unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("gt/train_2ch_con", con_2ch.unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("predicted/train_4ch", torch.from_numpy(la_4ch_pred_t).unsqueeze(0).unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("gt/train_4ch_con", con_4ch.unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)

        writer.add_images("predicted/train_sa_mid_weighted", torch.from_numpy(sa_mid_pred_weighted_t).unsqueeze(0).unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("predicted/train_2ch_weighted", torch.from_numpy(la_2ch_pred_weighted_t).unsqueeze(0).unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("predicted/train_4ch_weighted",
                          torch.from_numpy(la_4ch_pred_weighted_t).unsqueeze(0).unsqueeze(1),
                          epoch * len(training_data_loader) + batch_idx)

        writer.add_images("predicted/sa_pred_ed", pred_sa_ed[:, :, 32, :, :],
                          epoch * len(training_data_loader) + batch_idx)
        writer.add_images("gt/sa_ed", img_sa_ed[:, 32:33, :, :],
                          epoch * len(training_data_loader) + batch_idx)


        if batch_idx % 40 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss all: {:.6f}, '
                  'Seg Loss: {:.6f}, Reg Loss: {:.6f}, Smooth Loss: {:.6f}, Huber Loss: {:.6f}, Myo Dice sa: {:.6f}, Myo Dice 2ch: {:.6f}, Myo Dice 4ch: {:.6f}'.format(
...
def val(epoch):
    ...
    for batch_idx, batch in tqdm(enumerate(val_data_loader, 1),
        ...
        with torch.no_grad():
            ...
            # faces_0 = faces.type(Tensor).cuda()  # [bs, number of faces, 3]
            ...
            # loss_reg = -torch.abs(delta_p).mean()
            # print (loss_reg)
            # print (loss_seg, loss_smooth)

            # define image registration as a regularization term

            loss_reg = flow_criterion(pred_sa_ed, x_sa_ed_5D)

            loss_huger = huber_loss_3d(net_sa['out'])

            # loss_all = loss_seg + w_smooth * loss_smooth
            loss_all = loss_seg + w_reg * loss_reg + w_smooth * loss_smooth + w_h * loss_huger



            bound_dist = torch.from_numpy(np.array([1])).cuda()
            # print (w_sa_t_4.shape)
            # print (v_sa_idx_t_4.shape)
            w_sa_4_sele_t = w_sa_t_4[0, :, 0][torch.abs(w_sa_t_4[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_sa_4_sele_x_t = v_sa_idx_t_4[0, :, 0][
                torch.abs(w_sa_t_4[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_sa_4_sele_y_t = v_sa_idx_t_4[0, :, 1][
                torch.abs(w_sa_t_4[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_sa_x_t = np.round(idx_sa_4_sele_x_t.detach().cpu().numpy()).astype(np.int16)
            idx_sa_y_t = np.round(idx_sa_4_sele_y_t.detach().cpu().numpy()).astype(np.int16)
            val_sa_t = w_sa_4_sele_t.detach().cpu().numpy()
            sa_mid_pred_t = np.zeros(shape=(height, width))
            sa_mid_pred_weighted_t = np.zeros(shape=(height, width))
            for i in range(val_sa_t.shape[0]):
                sa_mid_pred_t[idx_sa_x_t[i], idx_sa_y_t[i]] = 1
                sa_mid_pred_weighted_t[idx_sa_x_t[i], idx_sa_y_t[i]] = sa_mid_pred_weighted_t[
                                                                           idx_sa_x_t[i], idx_sa_y_t[i]] + \
                                                                       val_sa_t[i]
            sa_mid_pred_weighted_t = sa_mid_pred_weighted_t / (np.max(sa_mid_pred_weighted_t) + 1e-6)

            w_2ch_sele_t = w_2ch_t[0, :, 0][torch.abs(w_2ch_t[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_2ch_sele_x_t = v_2ch_idx_t[0, :, 0][torch.abs(w_2ch_t[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_2ch_sele_y_t = v_2ch_idx_t[0, :, 1][torch.abs(w_2ch_t[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_2ch_x_t = np.round(idx_2ch_sele_x_t.detach().cpu().numpy()).astype(np.int16)
            idx_2ch_y_t = np.round(idx_2ch_sele_y_t.detach().cpu().numpy()).astype(np.int16)
            val_2ch_t = w_2ch_sele_t.detach().cpu().numpy()
            la_2ch_pred_t = np.zeros(shape=(height, width))
            la_2ch_pred_weighted_t = np.zeros(shape=(height, width))
            for i in range(val_2ch_t.shape[0]):
                la_2ch_pred_t[idx_2ch_x_t[i], idx_2ch_y_t[i]] = 1
                la_2ch_pred_weighted_t[idx_2ch_x_t[i], idx_2ch_y_t[i]] = la_2ch_pred_weighted_t[
                                                                             idx_2ch_x_t[i], idx_2ch_y_t[i]] + \
                                                                         val_2ch_t[i]
            la_2ch_pred_weighted_t = la_2ch_pred_weighted_t / (np.max(la_2ch_pred_weighted_t) + 1e-6)

            w_4ch_sele_t = w_4ch_t[0, :, 0][torch.abs(w_4ch_t[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_4ch_sele_x_t = v_4ch_idx_t[0, :, 0][torch.abs(w_4ch_t[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_4ch_sele_y_t = v_4ch_idx_t[0, :, 1][torch.abs(w_4ch_t[0, :, 0]) > torch.exp((-1) * temper * bound_dist)]
            idx_4ch_x_t = np.round(idx_4ch_sele_x_t.detach().cpu().numpy()).astype(np.int16)
            idx_4ch_y_t = np.round(idx_4ch_sele_y_t.detach().cpu().numpy()).astype(np.int16)
            val_4ch_t = w_4ch_sele_t.detach().cpu().numpy()
            la_4ch_pred_t = np.zeros(shape=(height, width))
            la_4ch_pred_weighted_t = np.zeros(shape=(height, width))
            for i in range(val_4ch_t.shape[0]):
                la_4ch_pred_t[idx_4ch_x_t[i], idx_4ch_y_t[i]] = 1
                la_4ch_pred_weighted_t[idx_4ch_x_t[i], idx_4ch_y_t[i]] = la_4ch_pred_weighted_t[
                                                                             idx_4ch_x_t[i], idx_4ch_y_t[i]] + \
                                                                         val_4ch_t[i]
            la_4ch_pred_weighted_t = la_4ch_pred_weighted_t / (np.max(la_4ch_pred_weighted_t) + 1e-6)

            dice_sa = categorical_dice(sa_mid_pred_t, contour_sa[0, 4, :, :].cpu().numpy())
            dice_2ch = categorical_dice(la_2ch_pred_t, contour_2ch[0, :, :].cpu().numpy())
            dice_4ch = categorical_dice(la_4ch_pred_t, contour_4ch[0, :, :].cpu().numpy())

            Myo_dice_sa.append(dice_sa)
            Myo_dice_2ch.append(dice_2ch)
            Myo_dice_4ch.append(dice_4ch)
            ...
            if batch_idx == 1:
                ...
                writer.add_images("predicted/val_sa_mid", torch.from_numpy(sa_mid_pred_t).unsqueeze(0).unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)
                writer.add_images("gt/val_sa_mid", con_sa[:, 4:5, :, :],
                                  epoch * len(training_data_loader) + batch_idx)
                writer.add_images("predicted/val_2ch", torch.from_numpy(la_2ch_pred_t).unsqueeze(0).unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)
                writer.add_images("gt/val_2ch_con", con_2ch.unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)
                writer.add_images("predicted/val_4ch", torch.from_numpy(la_4ch_pred_t).unsqueeze(0).unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)
                writer.add_images("gt/val_4ch_con", con_4ch.unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)

                writer.add_images("predicted/val_sa_mid_weighted",
                                  torch.from_numpy(sa_mid_pred_weighted_t).unsqueeze(0).unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)
                writer.add_images("predicted/val_2ch_weighted",
                                  torch.from_numpy(la_2ch_pred_weighted_t).unsqueeze(0).unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)
                writer.add_images("predicted/val_4ch_weighted",
                                  torch.from_numpy(la_4ch_pred_weighted_t).unsqueeze(0).unsqueeze(1),
                                  epoch * len(training_data_loader) + batch_idx)
    ...
    # print('Loss all: {:.6f}, '
    #       'Seg Loss: {:.6f}, Smooth Loss: {:.6f}, Myo Dice sa: {:.6f}, Myo Dice 2ch: {:.6f}, Myo Dice 4ch: {:.6f}'.format(
    #     np.mean(val_loss),
    #     np.mean(val_seg_loss), np.mean(val_smooth_loss), np.mean(Myo_dice_sa), np.mean(Myo_dice_2ch),
    #     np.mean(Myo_dice_4ch)))
...
# data_path = '/vol/biomedic3/qm216/multimesh/train/train_sample'
...
# val_data_path = '/vol/biomedic3/qm216/multimesh/train/train_sample'
